...
Identifier convention: roadmap recommendations are `REC-R0`–`REC-R8`;
first-principles rejections are `REJ-R1`–`REJ-R3`. Legacy bare `R#`
references are noncanonical because the two namespaces overlap.

## Immediate (this week)

### REC-R0 — Housekeeping that unblocks everything
...
### REC-R1 — `vh shrink` CLI wiring (M4a)
...
### REC-R2 — Swarm masks (M2a, the 20-line multiplier)
...
### REC-R3 — Decision tape + PCT schedule perturbation (M2b)
...
### REC-R4 — Evidence store + regression corpus (M4c)
...
### REC-R5 — Provider/tool gremlin family + cassette boundary spike (M3a)
...
### REC-R6 — Tier-2 subprocess universes (the active goal doc, executed)
...
- Impact: very high (mission reach). Confidence: medium (process-boundary determinism without interposition is the core research risk — REJ-R1's falsifier lives here). Effort: 4–8 weeks. Risk: high (silent leaks = false CLEAN — the worst outcome this repo's epistemics exist to prevent). Reversibility: Tier-1 unaffected.
...
### REC-R7 — Corpus harvesting pipeline
Harvest ≥10 (toward ≥25) real bugs: dogfood on dharma_swarm (`clients/python` Phase-4 hook) and 2–3 OSS vibe-coded repos; entries cite replay bundles (REC-R4) and pass the §5 typed admission gate (Reproduced + PromotionProof only).
...
## Longer horizon (post-90d, gated on REC-R5–REC-R7 evidence)
...
- **Assertion-novelty guidance** (sometimes-hit bitmaps feeding mask allocation) — only after REC-R2/REC-R3 ablations show guidance headroom remains.
...
- **OSS positioning**: LICENSE + public corpus + honest bakeoff vs Turmoil/MadSim published; Antithesis-style OSS free-tier analog is premature until REC-R7.
- **Not on any horizon**: hypervisor (REJ-R1), RL exploration (REJ-R3), eval dashboards, multi-LLM sign-off gates.

## First three implementation-ready spike specs

### RFC-001 — `vh shrink` CLI (REC-R1)
...
- Gates: A/B bakeoff harness in `scripts/` with the REC-R2 acceptance test as a CI-reportable (initially non-blocking) job; frozen palette v0 untouched.
- Rollback: default flag flip.

### RFC-003 — Decision tape (REC-R3a, prerequisite of PCT and causality)
...
REC-R0 → REC-R1, REC-R2 (parallel) → REC-R4 → REC-R3 (tape before PCT) → REC-R5 (cassette spike, parallel with REC-R3) → REC-R6, REC-R7 → REC-R8. REC-R2/REC-R3 ablation results gate all "guided exploration" claims; REC-R5/REC-R6 results gate all "agent-system testing" claims; REC-R7 results gate the OSS-positioning story.

## Acceptance test and kill criterion index

Every item above carries a machine-checkable `Acceptance test:` line and a `Kill criterion:` line at line start (REC-R0–REC-R8). The three implementation-ready spikes (RFC-001/002/003) inherit the acceptance tests of their parent items (REC-R1, REC-R2, REC-R3).
