...
fn main() -> Result<(), SoakFailed> {
    ...
    // replay input, event, property, trace hash, or Tier-1 identity.
...
mod tests {
    ...
    fn panic_is_converted_to_a_frozen_error_receipt() {
        ...
        assert_eq!(
            ...
            "soak: receipt-schema=vh-verify-soak-v1 verdict=ERROR determinism-tier=Tier-1 evidence-grade=D0 root-seed=0x000000000000d1ce universe=0 workload=vh-verify-reference trace-format=v0 observable-schema=vh-verify-observable-v3 requested-runs=7 error-code=panic panic-stage=cli-boundary panic-run=boundary"
        );
    }
}
