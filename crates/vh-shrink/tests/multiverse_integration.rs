...
use vh_multiverse::{
    run_universe_with_fault_plan, FaultPlanDiscipline, PropertyContract, RunOutcome, UniverseCtx,
...
fn replay_pair_reproduces_expected_failure(
    ...
    first.observably_equal(replay)
        ...
        && reproduces_expected_failure(first)
}
...
fn shrinker_minimizes_through_the_public_fault_plan_replay_hook() {
    let input = FaultPlan::new(vec![
        ...
        injection(30, FaultKind::NetworkDuplicate),
        injection(40, FaultKind::NetworkReorder),
        injection(50, FaultKind::TornWrite),
        injection(60, FaultKind::FsyncLie),
    ]);
    ...
    assert_eq!(report.original_injections(), 9);
...
struct ReportsFailureWithoutRetrievingOverride;

impl Workload for ReportsFailureWithoutRetrievingOverride {
    fn name(&self) -> &str {
...
fn expected_failure_with_invalid_replay_lifecycle_is_not_a_reproducer() {
    let result = run_universe_with_fault_plan(
        SEED,
        UNIVERSE,
        &ReportsFailureWithoutRetrievingOverride,
...
impl Workload for ReportsSameNameWithDifferentDetail {
    ...
    fn run(&self, ctx: &mut UniverseCtx) -> RunOutcome {
        let _plan = ctx.fault_plan_or(FaultPlan::default);
...
fn same_property_name_with_different_detail_is_not_the_expected_fingerprint() {
    ...
    assert!(result.always_failures().iter().any(|failure| {
        failure.name == PROPERTY && failure.detail == "different causal detail"
    }));
    assert!(!reproduces_expected_failure(&result));
}

#[test]
fn full_observable_divergence_is_not_a_reproducer_even_when_failure_matches() {
